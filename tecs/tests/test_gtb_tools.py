...
class TestGtbTools(unittest.TestCase):
    def test_find_xyz_file(self):
        """Test gtb.tools.find_xyz_file()"""
        files = (
        ...
        for (obs_name, xyz_name) in files:
            ...
            try:
                ...
                self.assertIsNotNone(f_file, f'XYZ file for the {os.path.basename(obs_file)} ({os.path.basename(xyz_file)}) not found.')
            finally:
                shutil.rmtree(tdir)

    def test_load_xyz_file(self):
        """Test gtb.tools.load_xyz_file()"""
        test_data = {
...
'''
        with tempfile.NamedTemporaryFile(mode='wt', delete=False) as fobj:
            fobj.write(comment)

            for dt in sorted(test_data):
                rec = '{} {} {} {}\n'.format(dt, *test_data[dt])
                fobj.write(rec)
            temp_filename = fobj.name

        try:
            loaded_data = load_xyz_file(temp_filename)
            ...
            for epoch in test_data:
                ...
                msg = f'{test} != {loaded}'
                self.assertEqual(test, loaded, msg)
        finally:
